...
export const getCompanies = async () => {
    ...
    return companies.map((company) => {
        const activeDeals = company.deals.filter((d) => !d.stage.includes('Closed'));
        const wonDeals = company.deals.filter((d) => d.stage === 'Closed Won');
        return {
            ...company,
            contactCount: company.contacts.length,
            activeDealCount: activeDeals.length,
            activeDealValue: activeDeals.reduce((sum, d) => sum + d.value, 0),
...
export const getCompanyInsights = async (id) => {
    const company = await prisma.company.findUnique({
        where: { id, isDeleted: false },
    });
    if (!company) {
...
export const enrichCompany = async (id) => {
    const company = await prisma.company.findUnique({
        where: { id, isDeleted: false },
    });
    if (!company) {
...
export const bulkEnrich = async (ids) => {
    let count = 0;
    for (const id of ids) {
        try {
