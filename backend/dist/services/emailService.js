...
export const emailService = {
    ...
    generateEmail: async (params) => {
        ...
        if (additionalContext)
            contextStr += `Additional Context: ${additionalContext}\n`;
        if (contactId) {
            ...
            if (contact) {
                ...
                if (contact.company)
                    contextStr += `Company: ${contact.company.name}\n`;
    ...
    improveEmail: async (params) => {
        ...
        switch (action) {
            case 'shorter':
                ...
                break;
            case 'formal':
                instruction = "Make this email more formal and professional.";
                break;
            case 'friendly':
                instruction = "Make this email friendlier and more conversational.";
                break;
            case 'grammar':
                instruction = "Fix any grammar and spelling mistakes in this email.";
                break;
            case 'cta':
                instruction = "Add a clear call-to-action to this email.";
                break;
            case 'translate':
                instruction = `Translate this email to ${language || 'English'}.`;
                break;
        }
        const llm = new Llm({ provider: process.env.LLM_PROVIDER });
        const prompt = `
