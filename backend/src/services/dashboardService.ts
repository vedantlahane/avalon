...
export const dashboardService = {
  getStats: async () => {
    ...
    leadScores.forEach(ls => {
      if (ls.leadCategory === 'Hot Lead') leadScoreDistribution[0].count = ls._count.id;
      if (ls.leadCategory === 'Warm Lead') leadScoreDistribution[1].count = ls._count.id;
      if (ls.leadCategory === 'Cool Lead') leadScoreDistribution[2].count = ls._count.id;
      if (ls.leadCategory === 'Cold Lead' || ls.leadCategory === 'Unqualified') leadScoreDistribution[3].count += ls._count.id;
    });
