...
export const reportController = {
  getSalesPerformance: catchAsync(async (c: Context) => {
    const data = await reportService.getSalesPerformance();
    return c.json(data);
  }),

  getPipelineAnalysis: catchAsync(async (c: Context) => {
    const data = await reportService.getPipelineAnalysis();
    return c.json(data);
  }),

  getActivityReports: catchAsync(async (c: Context) => {
    const data = await reportService.getActivityReports();
    return c.json(data);
  }),

  getContactAnalytics: catchAsync(async (c: Context) => {
    const data = await reportService.getContactAnalytics();
    return c.json(data);
  }),

  getAiInsights: catchAsync(async (c: Context) => {
    const data = await reportService.getAiInsights();
    return c.json(data);
  }),

  queryAi: catchAsync(async (c: Context) => {
    ...
    const llm = new Llm({ provider: process.env.LLM_PROVIDER as any });
