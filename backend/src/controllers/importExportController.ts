...
export const analyzeImport = catchAsync(async (c: Context) => {
...
export const previewImport = catchAsync(async (c: Context) => {
  const { data, mapping } = await c.req.json() as { data: any[], mapping: Record<string, string> };

  if (!data || !mapping) {
    throw new ApiError(400, 'data and mapping are required');
  }

  const preview = data.slice(0, 5).map((row: any) => {
    const mappedRow: any = {};
...
export const executeImport = catchAsync(async (c: Context) => {
  const body = await c.req.json();
  const result = await importExportService.executeImport(body);
  return c.json(result);
});

export const exportData = catchAsync(async (c: Context) => {
  const resource = c.req.query('resource') as string;
  const ids = c.req.query('ids') as string;
  const format = c.req.query('format') as string;
  const fields = c.req.query('fields')?.split(',') as string[];
