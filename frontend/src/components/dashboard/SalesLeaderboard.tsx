...
export const SalesLeaderboard: React.FC<SalesLeaderboardProps> = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
      {/* Leaderboard Card */}
      <div className="lg:col-span-6 bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className={cn("text-sm font-bold text-foreground", rep.isCurrentUser && "text-primary")}>
                      {rep.name} {rep.isCurrentUser && "(You)"}
                    </span>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-muted-foreground font-medium">{rep.dealsWon} deals won</span>
                      <span className="w-1 h-1 rounded-full bg-border"></span>
                      <span className="text-xs text-muted-foreground font-medium">Target: {rep.target}%</span>
                      {getStatusIcon(rep.status)}
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-sm font-bold text-foreground">{formatCurrency(rep.revenue)}</span>
                </div>
              </div>
              <div className="relative h-2 w-full bg-muted rounded-full overflow-hidden">
                <div 
                  className={cn(
                    ...
                    rep.rank === 1 ? "bg-amber-400" : rep.isCurrentUser ? "bg-primary" : "bg-muted-foreground/30"
                  )}
      ...
      <div className="lg:col-span-4 space-y-8">
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {data.achievements.map((badge) => (
              <div 
                key={badge.id} 
                className="p-4 rounded-xl border border-border bg-muted/30 flex flex-col items-center text-center group hover:bg-card hover:shadow-md transition-all"
              >
                <span className="text-3xl mb-2 transition-transform group-hover:scale-125 duration-300">{badge.icon}</span>
                <span className="text-sm font-bold text-foreground">{badge.title}</span>
                <span className="text-[10px] font-medium text-muted-foreground mt-1">{badge.description}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden relative group">
          <div className="absolute top-0 right-0 p-4">
            <Brain size={24} className="text-primary opacity-20 group-hover:opacity-40 transition-opacity" />
          </div>
          <div className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-foreground">AI Coaching</h3>
            </div>
            <p className="text-sm text-foreground/80 leading-relaxed italic">
              "🤖 {data.aiCoaching}"
            </p>
            <div className="mt-6 flex gap-3">
              <button className="flex-1 py-2 text-xs font-bold text-primary-foreground bg-primary rounded-lg shadow-md shadow-primary/20 hover:bg-primary/90 transition-all">
                View Recommendations
              </button>
              <button className="flex-1 py-2 text-xs font-bold text-primary border border-primary/20 rounded-lg hover:bg-primary/5 transition-all">
                Dismiss
              </button>
            </div>
          </div>
